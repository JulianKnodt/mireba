...
impl<D> Simple<D> {
  ...
  pub fn new(albedo: Vec3<D>) -> Self { Simple{albedo} }
}
impl<D: Float> Material<D> for Simple<D> {
  fn scatter(&self, _: &Ray<D>, vis: &Visibility<D>) -> Option<(Vec3<D>, Ray<D>)> {
    Some((self.albedo, Ray::new(vis.pos, vis.norm)))
  }
}
  ...
  Simple<D>: Material<D>,
{
  fn scatter(&self, r: &Ray<D>, vis: &Visibility<D>) -> Option<(Vec3<D>, Ray<D>)> {
    match self {
      ...
      Mat::Simple(ref s) => s.scatter(r, vis),
...
mat_from!(D, Simple<D>, Mat::Simple);
